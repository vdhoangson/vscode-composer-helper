...
import { CommandNames } from "../util/commands";
...
import { ComposerError } from "../util/errors";
...
export class ComposerExtension extends Disposable {
  ...
  private runningProcesses: Map<number | undefined, Process> = new Map();
  ...
  private registerCommands(): void {
    ...
    this.registerCommand(
      CommandNames.DumpAutoload,
      this.runCommandDumpAutoload
    );
  ...
  protected runCommandArchive(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerArchiveInput,
        placeHolder: Strings.ComposerArchivePlaceHolder,
      })
      .then((pkg) => {
        if (typeof pkg !== "undefined") {
          const args = pkg !== String.Empty ? pkg.split(String.Space) : [];
  ...
  protected runCommandCreateProject(_context: ComposerContext): void {
    ...
    throw new ComposerError({
      message: String.format(
        Strings.ComposerCommandNotImplemented,
        "create-project"
      ),
    });
  }
  ...
  protected runCommandDepends(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerDependsInput,
        placeHolder: Strings.ComposerDependsPlaceHolder,
      })
      .then((options) => {
  ...
  protected runCommandDumpAutoload(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerDumpAutoloadInput,
        placeHolder: Strings.ComposerDumpAutoloadPlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined") {
          const args =
            options !== String.Empty ? options.split(String.Space) : [];
  ...
  protected runCommandInit(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerInitInput,
        placeHolder: Strings.ComposerInitPlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined" && options !== String.Empty) {
          const args = options.split(String.Space);
  ...
  protected runCommandProhibits(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerProhibitsInput,
        placeHolder: Strings.ComposerProhibitsPlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined" && options !== String.Empty) {
          const args = options.split(String.Space);
  ...
  protected runCommandRemove(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerRemoveInput,
        placeHolder: Strings.ComposerRemovePlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined" && options !== String.Empty) {
          const args = options.split(String.Space);
  ...
  protected runCommandRequire(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerRequireInput,
        placeHolder: Strings.ComposerRequirePlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined" && options !== String.Empty) {
          const args = options.split(String.Space);
  ...
  protected runCommandRunScript(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerRunScriptInput,
        placeHolder: Strings.ComposerRunScriptPlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined" && options !== String.Empty) {
          const args = options.split(String.Space);
  ...
  protected runCommandShow(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerShowInput,
        placeHolder: Strings.ComposerShowPlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined") {
          const args =
  ...
  protected runCommandSuggests(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerSuggestsInput,
        placeHolder: Strings.ComposerSuggestsPlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined") {
          const args =
  ...
  protected runCommandWhy(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerWhyInput,
        placeHolder: Strings.ComposerWhyPlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined" && options !== String.Empty) {
          const args = options.split(String.Space);
  ...
  protected runCommandWhyNot(context: ComposerContext): void {
    window
      .showInputBox({
        prompt: Strings.ComposerWhyNotInput,
        placeHolder: Strings.ComposerWhyNotPlaceHolder,
      })
      .then((options) => {
        if (typeof options !== "undefined" && options !== String.Empty) {
          const args = options.split(String.Space);
  ...
  protected registerCommand(
    command: string,
    callback: ComposerCommandHandler,
    thisArg?: any
  ): void {
    const contextCallback = this.ensureComposerContext(
      this.safeExecute(callback)
    );
    this.disposables.push(
      commands.registerCommand(command, contextCallback, thisArg)
    );
  }
  ...
  protected runCommand(
    ...
    ensureComposerProject: boolean = true
  ): void {
    ...
    if (
      context.settings.ignorePlatformReqs &&
      args[0] &&
      [
        "dump-autoload",
        "install",
  ...
  private runCommandInOutputWindow(
    args: string[],
    context: ComposerContext
  ): void {
    ...
    const commandString = [
      context.settings.executablePath,
      ...commandArgs,
    ].join(" ");
    const commandProcess: cp.ChildProcess = cp.exec(commandString, {
      cwd: context.workingPath,
      env: process.env,
    });

    this.runningProcesses.set(commandProcess.pid, {
    ...
    this.outputChannel.appendLine(
      String.format(Strings.WorkingDirectory, context.workingPath)
    );
    this.outputChannel.appendLine(
      String.format(Strings.ExecutingCommand, commandArgs.join(" "))
    );
    this.outputChannel.appendLine("");
  }

  private runCommandInTerminal(args: string[], context: ComposerContext): void {
    runInTerminal(context.settings.executablePath, args, {
      cwd: context.workingPath,
      env: process.env,
    });
  }

  private runCommandInIntegratedTerminal(
    args: string[],
    context: ComposerContext
  ): void {
    if (!this.terminal) {
   ...
   */
  private ensureComposerContext(
    callback: ComposerCommandHandler
  ): ComposerCommandHandler {
        ...
        default:
          window
            .showWorkspaceFolderPick({
              placeHolder: Strings.WorkspaceFolderPick,
            })
            .then((folder: any) => {
