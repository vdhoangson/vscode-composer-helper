...
export class ComposerContext {
  ...
  private _onDidChangeSettings =
    new EventEmitter<ComposerSettingsChangeEvent>();
  ...
  public get workingPath(): string {
    ...
    // Process settings.
    const settingsPath = this.settings?.workingPath || null;
   ...
   */
  public get composerJsonPath(): string | null {
    try {
      const composerJsonPath = fs.realpathSync(
