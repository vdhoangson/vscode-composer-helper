...
export const packageHoverProvider = vscode.languages.registerHoverProvider(
  globals.documentSelector,
  {
    async provideHover(
      ...
      const isInsideDependencies = new RegExp(
        `"(require|require-dev)":\\s*?\\{[^{}]*?"${packageName.replace(
          /\//g,
          "\\/"
        )}"[^{}]*?\\}`,
        "gm"
      ...
      const latestPackage = await PackagistProvider.getLatestPackage(
        packageName
      );
      const markDown = await getMarkdownStringNew(
        latestPackage,
        installedPackage
      );
...
const getMarkdownStringNew = async function (
  latestPackage: types.P2Package,
  ...
  return new vscode.MarkdownString()
    .appendMarkdown("**VS Composer Intellisense** \n\n")
    .appendMarkdown("------------------------------ \n\n")
    .appendMarkdown("**" + latestPackage.name + "** \n\n")
    .appendMarkdown(latestPackage.description + "\n\n")
    .appendMarkdown(`Author: ${latestPackage.authors[0].name}` + "\n\n")
    .appendMarkdown(`Latest version: ${latestPackage.version}` + "\n\n")
    .appendMarkdown(`Installed version: ${installedVersion}` + "\n\n")
    .appendMarkdown(
      `[Packagist](https://packagist.org/packages/${latestPackage.name})`
    )
    .appendText(" | ")
    .appendMarkdown(`[Source](${latestPackage.source.url})` + "\n\n");
};
